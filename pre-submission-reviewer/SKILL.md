...
- Experiments validate the paper's main claims, not tangential
  ones.
- Experimental comparisons use matched tasks, datasets, seeds,
  checkpoints, budgets, and protocols, or state the mismatch.
- Result prose reports variation and exceptions rather than
  relying only on pooled means.
- Mechanistic or causal explanations are supported by controlled
  comparisons rather than inferred from aggregate differences.
