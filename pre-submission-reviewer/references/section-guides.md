...
- Observation units, seed counts, aggregation axes, and missing
  cells are stated or recoverable from the text and tables.

Overall comparison:

- End-to-end comparison against all baselines first.
- State the metric before analysing results.
- Compare methods on a common evaluation subset.
- State the direction and magnitude of the result.
- Report important variation, exceptions, and coverage limits.
- Treat an explanation for the result as a hypothesis unless a
  controlled comparison isolates it.
...
- Keep all non-ablated settings fixed before attributing a change
  to one component.

Must-have:

- Real datasets and persuasive baselines.
- Conclusions stated explicitly and at the strength supported by
  the design.
- Exact scopes for “best,” “consistent,” “robust,” and
  “generalizes.”
- No statistical-significance claim without a specified test.
- No table-by-table number dump or generic “these findings
  highlight” conclusion.

When result files or tables are available, use the
`experiment-data-analysis` skill to audit coverage, aggregation,
comparison fairness, and claim strength before judging the prose.
