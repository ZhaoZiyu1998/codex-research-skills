...
4. **Empirical Findings**. Multi-angle comparisons that identify capability
boundaries. State the comparison scope, decisive evidence, variation, and
exceptions. Do not force results into promotional or formulaic “Finding X”
statements. Deep dive: `references/experiments.md`.
...
- **§5 Experiments**: organized by RQ. Include the Overall Performance table
  (typically the largest table in the paper), fine-grained analysis, a human
  baseline when available, and evidence-led summaries with explicit scope and
  exceptions. See `references/experiments.md`. Use the
  `experiment-data-analysis` skill when converting result files or tables into
  claims and prose.
